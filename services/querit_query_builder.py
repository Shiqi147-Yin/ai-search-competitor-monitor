"""English query generator with source-targeted retrieval.

Generates English search queries from YAML templates.
All queries must be in English.
The "All" mode is split into multiple source-specific queries
instead of using one broad query.
"""
...
_TEMPLATES_PATH = (
    ...
    / "config"
    / "querit_query_templates.example.yaml"
)

_SOURCES_PATH = (
    Path(__file__).parent.parent
    / "config"
    / "competitor_sources.example.yaml"
)
...
def _resolve_template_vars(
    ...
    """Fill template variables including domain and GitHub owner."""

    official_domains = competitor_sources.get("official_domains", [])

    docs_domain = next(
        (d for d in official_domains if "docs." in d),
        official_domains[0] if official_domains else "",
    )

    main_domain = (
        official_domains[0]
        if official_domains
        else competitor.lower() + ".com"
    )
    ...
    result = (
        ...
        .replace(
            "{competitor_docs_domain}",
            docs_domain or main_domain,
        )
...
def _add_date_range_suffix(
    query: str,
    query_type: str,
    date_range: dict,
) -> str:
    """Append an absolute date range to non-targeted templates."""
    ...
    if query_type in ("github", "github_targeted"):
        suffix = (
            f" Focus on content committed or released between "
            f"{start} and {end}. "
        ...
        suffix = (
            f" Focus on events announced or held between "
            f"{start} and {end}. "
        ...
        suffix = (
            f" Prioritize content published between "
            f"{start} and {end}. "
...
def generate_queries(
    ...
    """Generate English queries with date and source targeting.

    "All" mode uses multiple source-specific queries rather
    ...
    if not query_types:
        query_types = [
            "official_blog_targeted",
            "official_docs_targeted",
            "github_targeted",
            "events_targeted",
            "social_targeted",
        ]
